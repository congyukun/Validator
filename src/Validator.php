    ...
    public function check($data, $rules = [], $scene = '')
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            if ($rule instanceof \Closure) {
                ...
                //  验证因子
                $result = $this->checkItem($key, $value, $rule->getRule(), $data, $rule->getTitle() ?: $title,
                    $rule->getMsg());
            } else {
    ...
    public function is($value, $rule, $data = [])
    {
        ...
        switch (lcfirst($rule)) {
            ...
            case 'file':
                ...
                break;
            case 'image':
                $result = $value instanceof SplFileObject && in_array($this->getImageType($value->getRealPath()),
                        [1, 2, 3, 6]);
                break;
            default:
